...
class Intregation_model extends CI_Model
{
	...
	public function store_setup_apicall_auto()
    {
		
		 $sql='select * from  rev_storinfo';
       // $query = $this->db->get_where('rev_storinfo',array('user_id' => $user_interanal_id));
	   $query = $this->db->query($sql);
	   $result=$query->result();
	  //echo "<pre>"; print_r( $result);
	   foreach($result as $storeinfo)
	   {
		   $storename = $storeinfo->store_name;
		     $key = $storeinfo->store_apikey;
			 $password = $storeinfo->store_app_password; 
			 $store_id=$storeinfo->id;
			 
			  $user_interanal_id=$storeinfo->user_id;
			 
			 $sql2='select * from  rev_user where user_interanal_id='.$user_interanal_id;
			   // $query = $this->db->get_where('rev_storinfo',array('user_id' => $user_interanal_id));
			   $query2 = $this->db->query($sql2);
			   $result2=$query2->result(); 
			   $user_email_id=$result2[0]->user_email_id;
		   ...
		   // $user_interanal_id=$user_info->user_interanal_id;
			 ...
			 foreach($res_products as $product)
			{
				...
				if(is_array($varient))
				{
					foreach($varient as $product_varient)
					{
						...
						$this->db->delete('rev_shopify_product', array('product_id' =>$product->id,'user_id'=>$user_interanal_id,'variants_id'=>$product_varient->id)); 
						$data = array(
							'product_id' =>$product->id,
							'title' =>$product->title,
							 'body_html' =>$product->body_html,
				...
				if(is_array($fulfillments))
				{
					 ...
					 foreach($fulfillments as $fulfillments_data)
					{
						...
						if(is_array($line_items))
				        {
							...
							foreach($line_items as $line_item)
							{
							$this->db->delete('rev_orders', array('order_id' =>$orders->id,'user_id'=>$user_interanal_id,'line_items_id'=>$line_item->id)); 
			...
			curl_close($ch); 
			 ...
			 $to =$user_email_id;
						$subject = "Inubaan Software Product - Automatic Product Setup  ";
		               $message ='<table width="480" cellspacing="0" cellpadding="0" border="0">
							...
							<p>Product/Order Setup Automaticaly Updated by Cron Job</p>
							<br>
	...
	//newaccess token
	/* EAADFZBIxwBE0BAI6yA8SemlzZCGk7poy58kFMrbr9odCQE7AZCcNsfEGavfmGq03KoarkiQrTgyMfdGiK2Yfpv9WUT615VJDUeRZCZAlDpx05ZBmUU9EuFUUnCdoQtPTVMyLcFvGDyZCObL0FLUgeHs07cK3ELhSQNHWw6Iwe7CzwrifD5ZA6nuojADeID2ZAETUZD */
	...
	/****link****/
	//https://developers.facebook.com/tools/explorer/217671298712653?method=GET&path=act_10154710589527380%2Fadsets&version=v2.9
	$url='https://graph.facebook.com/v2.8/act_'.$ads_act.'/adsets?fields=account_id,daily_budget,name,campaign_id,budget_remaining,optimization_goal,created_time,end_time,start_time,status,ads{adset,adset_id,name,insights{ad_name,clicks,cpc,cpm,cpp,ctr,impressions,spend,cost_per_10_sec_video_view,cost_per_inline_link_click,cost_per_inline_post_engagement,cost_per_unique_click,cost_per_unique_inline_link_click,cost_per_action_type,cost_per_estimated_ad_recallers,cost_per_total_action,cost_per_unique_action_type}}&access_token=EAADFZBIxwBE0BAMA0VyvtDR4XO07KrYKhSPl64b3XBvGlTc3NUZAwRvEq8XZAOH00M84R2l70ccyQnKc2JZBhkaFeizzoR1zI0lM863AZAPyZCqOXsgQlPZC7obGxJVwQWI1rZBHhGJes2VZBajQLTk7ZAz0uVGZBuMOpHpWMNavpftegZDZD';
	...
	public function fb_setup_auto()
	{
		
		//genarate longtime fb token
		/* https://graph.facebook.com/oauth/access_token?client_id=217671298712653&client_secret=4e6028af9d5ddfc570d146a1c3df6200&grant_type=fb_exchange_token&fb_exchange_token=EAADFZBIxwBE0BAPdI4wHqZAFZBnJbdpDSnjr6LJ2ZAoR0YSqIi4wGdAZABiR5314lYclwKPao0NZCtlsUPAVVAiarHdLwT3DN6ZCus8WzYbkW3PFFUbF96w700FMt4bb5jWoikYLnZAh1kHz3dBAj3E5hlS2rn1lZA8UmCSc9VBVauXR68OIHDsPog7CjXTfLf4oZD */
		
		
		
		$sql='select * from  rev_facebook_info';
       // $query = $this->db->get_where('rev_storinfo',array('user_id' => $user_interanal_id));
	   $query = $this->db->query($sql);
	   $result=$query->result();
	 // echo "<pre>"; print_r( $result);exit;
	   foreach($result as $fbaccountinfo)
	   {
		   $ads_act=$fbaccountinfo->ads_act;
		   $user_id=$fbaccountinfo->user_id;
		   $fb_account_id=$fbaccountinfo->id;
		   
		  $sql2='select * from  rev_user where user_interanal_id='.$user_id;
       // $query = $this->db->get_where('rev_storinfo',array('user_id' => $user_interanal_id));
	   $query2 = $this->db->query($sql2);
	   $result2=$query2->result(); 
	   $user_email_id=$result2[0]->user_email_id;
		   ...
		   /* echo $url='https://graph.facebook.com/v2.8/act_'.$ads_act.'/adsets?fields=account_id,daily_budget,name,campaign_id,budget_remaining,optimization_goal,created_time,end_time,start_time,status,ads{adset,adset_id,name,insights{ad_name,clicks,cpc,cpm,cpp,ctr,impressions,spend,cost_per_10_sec_video_view,cost_per_inline_link_click,cost_per_inline_post_engagement,cost_per_unique_click,cost_per_unique_inline_link_click,cost_per_action_type,cost_per_estimated_ad_recallers,cost_per_total_action,cost_per_unique_action_type}}&access_token=EAADFZBIxwBE0BALos3j0EwgBaASmdG00ImEW0npcXRSu3ZCIX0ZAnY6vgKx9q3DeJicTUg3jPLJCBIi181KjuEZAUBxzXpOe5HC0g6ZBkRYy79yHfmN93LnlVddbGHefhDt8ZAp0UAmg73OeCbjmGVISZB4NUTmyZCAZD'; */
			 $url='https://graph.facebook.com/v2.8/act_'.$ads_act.'/adsets?fields=account_id,daily_budget,name,campaign_id,budget_remaining,optimization_goal,created_time,end_time,start_time,status,ads{adset,adset_id,name,insights{ad_name,clicks,cpc,cpm,cpp,ctr,impressions,spend,cost_per_10_sec_video_view,cost_per_inline_link_click,cost_per_inline_post_engagement,cost_per_unique_click,cost_per_unique_inline_link_click,cost_per_action_type,cost_per_estimated_ad_recallers,cost_per_total_action,cost_per_unique_action_type}}&access_token=EAADFZBIxwBE0BAMA0VyvtDR4XO07KrYKhSPl64b3XBvGlTc3NUZAwRvEq8XZAOH00M84R2l70ccyQnKc2JZBhkaFeizzoR1zI0lM863AZAPyZCqOXsgQlPZC7obGxJVwQWI1rZBHhGJes2VZBajQLTk7ZAz0uVGZBuMOpHpWMNavpftegZDZD';
						 ...
						 if(is_array($ads->insights->data))
							{
								...
								$sql = $this->db->set($data)->get_compiled_insert('rev_facebook_ads_campain'); 
									...
									//$to ='samiran1109@gmail.com,'. $user_email_id;
						$to =$user_email_id;
						$subject = "Inubaan Software Product - Automatic Facebook Ads Setup  ";
		               $message ='<table width="480" cellspacing="0" cellpadding="0" border="0">
							<tbody><tr>
							<td valign="top" align="center">
							<table style="border-bottom:1px solid #eee;padding:0 16px" width="480" height="50" cellspacing="0" cellpadding="24" border="0">
							...
							<td style="background-color:#ffffff" valign="top">
							<font style="font-size:16px;color:#444;">
							<p>Facebook Ads Campaign Setup Automaticaly Updated by Cron Job</p>
							<br>
