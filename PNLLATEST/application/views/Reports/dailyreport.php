	  ...
	  <div id="page-wrapper">
             ...
             <div class="panel-body table-responsive" style="display: block;">        
				...
				<table class="tablev  midfont">
				  ...
				  <tr class="tblma">
					...
					<th class="rpttext" title="{{item.product}}" ng-repeat="item in items">{{item.product |limitTo: 8}}</th>
        ...
        <tbody>  
			  <tr ng-if="value.orders!=0" ng-repeat="value in dailyreports">
				...
				<td class="Ads" ng-if="value.fb_conversion_inner" >{{value.fb_conversion_inner | currency}}</td> 
				<td class="Ads" ng-if="!value.fb_conversion_inner" >$0</td>
				<td>{{value.suppliertotal_cost | currency}}</td> 
				<td>{{value.fulfillment | currency}}</td> 
				
				<td ng-if="value.stripe_order!='0'">{{value.stripeorder | currency}}</td> 
				<td ng-if="value.stripe_order=='0'">{{value.stripeorder | currency}}</td> 
				
				<td ng-if="value.mis_inner">{{value.mis_inner | currency}}</td>
				<td ng-if="!value.mis_inner">$0</td> 
